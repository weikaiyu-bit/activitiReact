...
class BamExceptionLogSearchBar extends Component {
  state = {
    formValues: {},
  };
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      this.props.onFind(1, pagination.pageSize, values);
    });
  };
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={4} sm={24}>
            <FormItem label="请求方式">
              {getFieldDecorator('method')(<Input placeholder="请求方式" />)}
            </FormItem>
          </Col>
          <Col md={4} sm={24}>
            <FormItem label="异常信息">
              {getFieldDecorator('message')(<Input placeholder="异常信息" />)}
            </FormItem>
          </Col>
          <Col md={4} sm={24}>
...
export default BamExceptionLogSearchBar;
