...
import { Button, Col, Form, Icon, Input, Row, DatePicker, Select } from 'antd';
...
class TrainingPlanSearchBar extends Component {
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      /* 经办单位格式转换 */
     /* alert("树形下拉框的值-->fieldsValue.handlingOrgName: "+fieldsValue.handlingOrgName);*/
      if(fieldsValue.handlingOrgName){
        fieldsValue.orgId = fieldsValue.handlingOrgName.value;
        fieldsValue.handlingOrgName = fieldsValue.handlingOrgName.label;
      }
      console.log("fieldsValue.orgId: "+fieldsValue.orgId);
      ...
      fieldsValue.publishTime=fieldsValue.publishTime!=null
        ...
        :null;
      if (err) return;
      console.log("fieldsValue Keys: "+Object.keys(fieldsValue)+" Values: "+Object.values(fieldsValue));
      this.setState({
        formValues: fieldsValue,
      });
      this.props.onFind(pagination.current, pagination.pageSize,
        fieldsValue);
    });
  };
        ...
        <div className={styles.searchBar}>
          <Row>
            <Col md={6} sm={24}>
              ...
              {/*<FormItem label="关键字" labelCol={{ span: 6 }}
                wrapperCol={{ span: 16 }}>
                {getFieldDecorator('keywords')(<Input placeholder="请输入培训计划标题/内容等关键字" />)}
              </FormItem>*/}
            </Col>
            <Col md={7} sm={24}>
              <span >
...
export default TrainingPlanSearchBar;
