...
import { List, Avatar, Input, Button, Icon, Progress, Tag, message } from 'antd';
...
export default class TaskListView extends Component {
  ...
  componentDidMount() {
    const { onFindProject, onFindMembers } = this.props;
    onFindProject();
    onFindMembers({});
  }
  ...
  render() {
    const { dataSource, tabKey, loading: { effects } } = this.props;
    ...
    return (
      <div>
        <div style={{ marginBottom: 12 }}>
          <Search style={{ marginBottom: 8 }} placeholder="请输入关键字" onSearch={this.onTaskSearch} />
          <span style={{ fontSize: 20 }}>
            <a title="过滤条件"><Icon type="filter" /></a>
            <a title="按开始日期排序"><Icon type="sort-ascending" /></a>
          </span>
          {tabKey === '1' || tabKey === '2' ? <Button style={{ float: 'right' }} onClick={this.showAddTaskModal}>新建任务</Button> : <></>}
        </div>
        { addData.visible && <JobTaskAddModal { ...addData }/>}
        <List
          itemLayout="horizontal"
          dataSource={dataSource}
          renderItem={item => this.renderItem(item)}
          loading={effects['myJobsModel/fetch']}
        />
      </div>
    );
  }
