...
import { Modal, Form, Input, Radio, Table, message, Checkbox, Cascader, Popconfirm, Card, TreeSelect, Icon, DatePicker, Tag, Row, Select, Col } from 'antd';
...
class ApplyTransfers extends Component {
  ...
  okHandler = () => {
    const {
      record: {
        workName, rank, rankTime, workTime, positionName, id, name, sex, birth, heducation,
      },
    } = this.props;
    const {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        const {
          afterOrgName, afterPositionName, docNo, memberCategory, reason,
          promotionAppointment, afterLevel, removalTime, removalNo, afterServicedTime,
        } = values;
        let afterOrgNameAdd = '';
        ...
        const pfileWorkRecord = {
          ...
          promotionAppointment: promotionAppointment ? promotionAppointment.label : '', // 选拔任用方式
          reason, // 职务变动原因综述
          afterLevel: afterLevel ? afterLevel.label : '', // 新职级
          // 调入开始时间
          // b4AppointmentTime:moment(values['range-time-picker'][0]).format('YYYY-MM-DD HH:mm:ss'),
          // 调入结束时间
          // afterservicedTime:moment(values['range-time-picker'][1]).format('YYYY-MM-DD HH:mm:ss'),
          afterServicedTime: afterServicedTime ? moment(afterServicedTime).format('YYYY-MM-DD HH:mm:ss') : '',
  ...
  renderPersonnelFiles() {
    ...
    const {
      personnelTransferModal: { tree },
      pfileWorkRecordModel: { recordCategoryTree, recordLevelTree },
      // loading: { effects },
    } = this.props;
    const { changeCodeFlag } = this.state;
    ...
    if (key === 'position') {
      catgPer = (
        <Form onSubmit={this.okHandler} name="validate_other" >
          ...
          {/* <Form.Item label="新职级" {...formItemLayout}>
            {getFieldDecorator('afterLevel')(
              <DictionaryTree {...recordLevel} />,
            )}
          </Form.Item> */}
  ...
  render() {
    const {
      title, visible,
      // personnelTransferModal: { tree },
      pfileWorkRecordModel: { data, total },
      loading: { effects },
    } = this.props;
...
export default Form.create()(ApplyTransfers);
