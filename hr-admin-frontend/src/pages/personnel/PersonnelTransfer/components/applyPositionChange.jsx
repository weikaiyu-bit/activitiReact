...
class applyPositionChangeModel extends Component {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        const pfileWorkRecord = {
          ...
          afterServicedTime: moment(afterServicedTime._d).format('YYYY-MM-DD HH:mm:ss'),
          // eslint-disable-next-line no-underscore-dangle
          removalTime: removalTime ? moment(removalTime._d).format('YYYY-MM-DD HH:mm:ss') : '',
          removalNo,
          isMainPosition, // 是否主职务
          isLeader, // 是否领导职务
          isLeaderMember, // 是否领导成员
  ...
  render() {
    ...
    const {
      ...
      visible,
      personnelTransferModal: { tree },
      pfileWorkRecordModel: { data, total, recordCategoryTree },
      loading: { effects },
    } = this.props;
...
export default Form.create()(applyPositionChangeModel);
