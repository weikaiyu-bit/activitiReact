import { list, add, update, remove, listOrgOrg, listOrg, findPositionLevel, findEducational, findAnnualAppraisalNarrative, findRewardsPunishments, findFamilyRelations } from './service';
import utils from '../../../dtsea/common/utils';

const Model = {
    namespace: 'registerAndEntryModal',
    state: {
        data: [],
        total: 0,
    },
    ...
    effects: {
        ...
        * add({ payload, callback }, { call, put, select }) {
            ...
            if (callback) callback(response);
            if (response.code === 0) {
                const { params } = yield select(state => state.registerAndEntryModal);
