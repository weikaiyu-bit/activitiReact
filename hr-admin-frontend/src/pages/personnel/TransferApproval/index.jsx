...
@connect(({ transferApprovalModal, loading }) => ({
  transferApprovalModal,
  loading,
}))
class TransferApprovalIndex extends Component {
  state = {
    selectedRowKeys: [],
    filter: {},
    pageNumber: 1,
  ...
  columns = [
    ...
    {
      title: '审核状态',
      dataIndex: 'approvedState',
      ellipsis: true,
      width: 150,
      render: approvedState => {
        switch (approvedState) {
          case '已批准':
            return <Tag style={{ width: '64px' }} color="#87d068">{approvedState}</Tag>;
          case '待审核':
            return <Tag style={{ width: '64px' }} color="#f50">{approvedState}</Tag>;
          case '已驳回':
            return <Tag style={{ width: '64px' }} color="#f50">{approvedState}</Tag>;
          default:
    ...
    },
    {
      ...
      ellipsis: true,
      width: 150,
      render: workTime => this.renderDate(workTime),
    },
    {
      title: '学历',
    ...
    // },
    {
      ...
      children: [
        ...
        },
        {
          ...
          key: 'b4ServicedTime',
          width: 150,
          render: b4ServicedTime => this.renderDate(b4ServicedTime),
        },
        {
          title: '聘任时间',
          ...
          width: 150,
          render: b4AppointmentTime => this.renderDate(b4AppointmentTime),
        },
      ],
    },
  ...
  findPage = (pageNumber, pageSize, filter) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'transferApprovalModal/fetch',
      payload: {
        ...filter,
        pageNumber,
        pageSize,
  ...
  add = (id, values) => {
    ...
    const { pageNumber, pageSize, filter } = this.state;
    dispatch({
      type: 'transferApprovalModal/add',
      payload: values,
      callback: response => {
  ...
  update = (id, values) => {
    ...
    dispatch({
      type: 'transferApprovalModal/update',
      payload,
      callback: response => {
        switch (response.code) {
          case 'SUCCESS':
  ...
  delete = ids => {
    ...
    const { pageNumber, pageSize, filter } = this.state;
    dispatch({
      type: 'transferApprovalModal/remove',
      payload: { ids },
      callback: response => {
        switch (response.code) {
          case ErrorCode.SUCCESS:
  ...
  render() {
    ...
    const {
      loading: { effects },
      form,
      transferApprovalModal: { data = [], total },
    } = this.props;
      ...
      <Card bordered={false}>
        <div>
          ...
          <Table
            bordered
            rowKey="id"
            loading={effects['transferApprovalModal/fetch']}
...
export default Form.create()(TransferApprovalIndex);
